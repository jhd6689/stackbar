...
paint_volume_icon(AppClient *client_entity, cairo_t *cr, Container *container) {
    ...
    if ((container->state.mouse_pressing || container->state.mouse_hovering)) {
        if (container->state.mouse_pressing) {
            // dye_surface(surface, config->color_volume_pressed_icon);  // disabled to keep icon colors
        } else {
            // dye_surface(surface, config->color_volume_hovered_icon);  // disabled to keep icon colors
        }
    } else {
        // dye_surface(surface, config->color_volume_default_icon);  // disabled to keep icon colors
    }
...
paint_arrow(AppClient *client, cairo_t *cr, Container *container) {
    ...
    if (data->surface) {
        ...
        if (container->state.mouse_pressing || container->state.mouse_hovering) {
            if (container->state.mouse_pressing) {
                // dye_surface(data->surface, config->color_apps_scrollbar_pressed_button_icon);  // disabled to keep icon colors
            } else {
                // dye_surface(data->surface, config->color_apps_scrollbar_hovered_button_icon);  // disabled to keep icon colors
            }
        } else {
            // dye_surface(data->surface, config->color_apps_scrollbar_default_button_icon);  // disabled to keep icon colors
        }
