...
paint_right_item(AppClient *client, cairo_t *cr, Container *container) {
    ...
    if (arrow_right_surface) {
        if (container->state.mouse_pressing) {
            // dye_surface(arrow_right_surface, config->color_search_content_left_set_active_button_icon_pressed);  // disabled to keep icon colors
        } else {
            // dye_surface(arrow_right_surface, config->color_search_content_left_set_active_button_icon_default);  // disabled to keep icon colors
        }
...
paint_open(AppClient *client, cairo_t *cr, Container *container) {
    ...
    if (open_surface) {
        // dye_surface(open_surface, config->color_search_accent);  // disabled to keep icon colors
        cairo_set_source_surface(cr,
                                 open_surface,
    ...
    if (auto *tab_group = container_by_name("tab_group", client->root)) {
        for (auto *tab: tab_group->children) {
            ...
            if (tab_data->name == active_tab && tab_data->surface) {
                // dye_surface(tab_data->surface, config->color_search_empty_tab_content_icon);  // disabled to keep icon colors
                cairo_set_source_surface(cr,
                                         tab_data->surface,
