...
paint_close(AppClient *client_entity, cairo_t *cr, Container *container) {
    ...
    if (data->surface && active) {
        if (container->state.mouse_pressing || container->state.mouse_hovering) {
            if (container->state.mouse_pressing) {
                // dye_surface(data->surface, config->color_windows_selector_close_icon_pressed);  // disabled to keep icon colors
            } else {
                // dye_surface(data->surface, config->color_windows_selector_close_icon_hovered);  // disabled to keep icon colors
            }
        } else {
            // dye_surface(data->surface, config->color_windows_selector_close_icon);  // disabled to keep icon colors
        }
